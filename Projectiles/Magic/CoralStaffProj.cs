...
namespace WiitaMod.Items.Weapons.Magic
{
        ...
        public override void AI()
        {
            if (Timer >= 50)
            {
                ...
                if (target.active && !target.friendly && target.CanBeChasedBy())
                {
                    ...
                    Projectile.velocity = (Projectile.velocity * 10f + targetPos * (turnSpeed - length * 0.15f)) / 21f;
    ...
    public class CoralStaffHold : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(staffTexture, player.Center + posOffset - Main.screenPosition, staffTexture.Frame(), lightColor * Projectile.Opacity, MathHelper.ToRadians(-45 * Projectile.spriteDirection), staffTexture.Size() * 0.5f, Projectile.scale, effects, 0);
            return false;
        }
