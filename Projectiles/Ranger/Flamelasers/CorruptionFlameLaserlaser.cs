...
namespace WiitaMod.Projectiles.Ranger.Flamelasers
{
    ...
    public class CorruptionFlameLaserlaser : ModProjectile
    {
        ...
        private void CustomStuff(Player player)
        {
            /*if (Main.myPlayer == player.whoAmI)
            {
                Projectile.NewProjectile(player.GetSource_FromThis(), Projectile.position, Projectile.velocity * 4, ProjectileID.Flames, Projectile.damage, 0, Main.myPlayer);
            }*/
        ...
        private void SpawnDusts(Player player)
        {
            ...
            for (int i = 0; i < 5 + 1; i++)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, DustID.CursedTorch, 0.0f, 0.0f, 100, new Color(), 2f)];
                dust.velocity *= 10f;
                dust.noGravity = true;
                ...
                {
                    dust = Main.dust[Dust.NewDust(Main.player[Projectile.owner].Center + 55 * unit, 8, 8, DustID.CursedTorch, 0.0f, 0.0f, 100, new Color(), 2f)];
        ...
        private void ChargeLaser(Player player)
        {
            ...
            else
            {
                ...
                for (int k = 0; k < chargeFact + 1; k++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, DustID.CursedTorch, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f)];
        ...
        private void UpdatePlayer(Player player)
        {
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                aim = Vector2.Normalize(Vector2.Lerp(Vector2.Normalize(Projectile.velocity), aim, 0.07f)); // last variable is the turn speed
                aim *= 1f;
