...
namespace WiitaMod.Prim
{
    public partial class PrimTrail
    {
        ...
        public class DefaultShader : ITrailShader
        {
            ...
            public void ApplyShader<T>(Effect effect, T trail, List<Vector2> positions, string esp, float progressParam)
            {
                if (effect.HasParameter("vnoise"))
                    effect.Parameters["vnoise"].SetValue(ModContent.Request<Texture2D>("WiitaMod/Utilities/Noise/vnoise", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);

                if (effect.HasParameter("noiseTexture"))
                    effect.Parameters["noiseTexture"].SetValue(ModContent.Request<Texture2D>("WiitaMod/Utilities/Noise/noise", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);

                if (effect.HasParameter("spotTexture"))
                    effect.Parameters["spotTexture"].SetValue(ModContent.Request<Texture2D>("WiitaMod/Utilities/Noise/Spot", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);

                if (effect.HasParameter("ripperTexture"))
                    effect.Parameters["ripperTexture"].SetValue(ModContent.Request<Texture2D>("WiitaMod/Textures/RipperSlug", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);
        ...
        protected void PrepareBasicShader()
        {
            ...
            if (Pixellated)
            {
                Helpers.SetBasicEffectMatrices(ref WiitaMod.primitives.pixelEffect, Main.GameViewMatrix.Zoom);
                basicEffect = WiitaMod.primitives.pixelEffect;
            }
            /*else if (this is IGalaxySprite)
            {
                Helpers.SetBasicEffectMatrices(ref WiitaMod.primitives.galaxyEffect, new Vector2(1));
                basicEffect = WiitaMod.primitives.galaxyEffect;
            }*/
            else
                basicEffect = WiitaMod.basicEffect;
